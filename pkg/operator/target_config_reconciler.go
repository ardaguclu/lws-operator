...
func (c *TargetConfigReconciler) sync(ctx context.Context, syncCtx factory.SyncContext) error {
	...
	webhookSecret, _, err := c.manageCertSecret(WebhookCertificateSecretName)
	if err != nil {
		return err
	}
	...
	metricsSecret, _, err := c.manageCertSecret(MetricsCertificateSecretName)
	if err != nil {
		return err
	}
...
func (c *TargetConfigReconciler) manageCertSecret(secretName string) (*corev1.Secret, bool, error) {
...
func (c *TargetConfigReconciler) manageServiceMonitor(ctx context.Context, ownerReference metav1.OwnerReference) (bool, error) {
	...
	service := resourceread.ReadServiceV1OrDie(bindata.MustAsset("assets/lws-controller-generated/v1_service_lws-controller-manager-metrics-service.yaml"))
	endpoints, found, err := unstructured.NestedSlice(required.Object, "spec", "endpoints")
	if !found || len(endpoints) == 0 || err != nil {
		return false, fmt.Errorf("%v: .spec.endpoints not found: %v", required.GetName(), err)
	}

	serverName, found, err := unstructured.NestedString(endpoints[0].(map[string]any), "tlsConfig", "serverName")
	if !found || err != nil {
		return false, fmt.Errorf("%v: .spec.endpoints.0.tlsConfig.serverName not found: %v", required.GetName(), err)
	}
	serverName = strings.Replace(serverName, "SERVICE_NAME", service.Name, 1)
	serverName = strings.Replace(serverName, "SERVICE_NAMESPACE", c.namespace, 1)
	err = unstructured.SetNestedField(endpoints[0].(map[string]any), serverName, "tlsConfig", "serverName")
	if err != nil {
		return false, err
	}

	err = unstructured.SetNestedSlice(required.Object, endpoints, "spec", "endpoints")
	if err != nil {
		return false, err
	}
